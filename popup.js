...
let okCount = 0, failCount = 0, total = 0, done = 0;
...
chrome.storage.local.get(['savedUrls', 'savedMessage'], res => {
  if (res.savedUrls) urlInput.value = res.savedUrls;
  msgInput.value = res.savedMessage !== undefined ? res.savedMessage : DEFAULT_MESSAGE;
  updateCharCount();
});

urlInput.addEventListener('input', () => chrome.storage.local.set({ savedUrls: urlInput.value }));
...
function parseUrls(raw) {
  return raw.split('\n').map(l => l.trim()).filter(Boolean).map(url => {
    try {
      const u = new URL(url);
      const isYT = u.hostname.includes('youtube.com') && u.searchParams.get('v');
...
btnReport.addEventListener('click', async () => {
  ...
  okCount = 0; failCount = 0; done = 0; total = valid.length;
  logList.innerHTML = '';
  ...
  if (invalid) addLog('error', `${invalid} linha(s) ignorada(s) — URL inválida`, '');
  ...
  for (const url of valid) {
    ...
    try {
      result = await new Promise(resolve => {
        chrome.runtime.sendMessage({ action: 'reportVideo', url, reasonData, userComment }, resolve);
      });
      if (chrome.runtime.lastError) result = { success: false, error: chrome.runtime.lastError.message };
    } catch (e) {
